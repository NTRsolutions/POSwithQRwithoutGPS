...
//            _PopulateLogValues(ex, "getGroupById");
    ...
    public void replaceNulls() {
        database = getWritableDatabase();
        Cursor cursor = database.rawQuery("UPDATE Groups SET GroupID = IfNull(GroupID,'0'), GroupName = IfNull(GroupName,''), DeviceID = IfNull(DeviceID,'0'), VillageID = IfNull(VillageID,'0'), programId = IfNull(ProgramId,'0'), CreatedBy = IfNull(CreatedBy,''), NewFlag = IfNull(NewFlag,'0'), VillageName = IfNull(VillageName,''), SchoolName = IfNull(SchoolName,'') ,sharedBy = IfNull(sharedBy,'') ,SharedAtDateTime = IfNull(SharedAtDateTime,'') ,appVersion = IfNull(appVersion,'') ,appName = IfNull(appName,'') ,CreatedOn = IfNull(CreatedOn,'')", null);
        cursor.moveToFirst();
        cursor.close();
        database.close();
    }
